---
epoch: "Rozbiory"
epoch_id: 5
chapter: "Praca organiczna"
chapter_id: 8
difficulty: "easy"
question_count: 5
created_at: "2026-05-05T10:15:00Z"
session_start: "2026-05-05T10:00:00Z"
session_end: "2026-05-05T10:30:00Z"
tokens_input: 3500
tokens_output: 2800
tokens_total: 6300
---

# Praca organiczna: Pytania poziom łatwy

## Kontekst historyczny

Praca organiczna to strategia działania narodu polskiego pod zaborami, rozwinięta szczególnie po upadku powstania styczniowego. Pozytywiści uważali, że społeczeństwo przypomina żywy organizm, dla którego niezbędny jest równomierny rozwój wszystkich warstw i dziedzin życia. Zamiast zrywów niepodległościowych, postulowano budowanie silnej gospodarki, oświaty i organizacji społecznych. Pionierem ruchu był Dezydery Chłapowski, który wprowadzał reformy w swoim majątku w Turwia już po 1815 roku. Ruch rozwijał się najwcześniej w zaborze pruskim, a później rozprzestrzenił na pozostałe zabory. Organicznicy tworzyli kółka rolnicze, biblioteki ludowe, towarzystwa naukowe i spółdzielnie. Dzięki ich działalności Polacy mogli przetrwać politykę germanizacji i rusyfikacji oraz zachować tożsamość narodową. Najważniejszymi organizacjami były Poznańskie Towarzystwo Przyjaciół Nauk (1857) i Spółka Akcyjna Bazar (1838).

---

## Pytanie 1

**Question ID:** Q-RO-PO-001

**Pytanie:** Kto był pionierem pracy organicznej na ziemiach polskich, który po 1815 roku wprowadzał reformy w swoim majątku w Turwia?

**A)** Dezydery Chłapowski
**B)** Karol Marcinkowski
**C)** Hipolit Cegielski
**D)** Edward Raczyński

**Poprawna odpowiedź:** A

**Wyjaśnienie:** Dezydery Chłapowski był pionierem pracy organicznej, który po powrocie z Wielkiej Brytanii w 1815 roku rozpoczął wdrażać reformy w swoim majątku w Turwia. Wprowadzał nowoczesne metody gospodarcze i edukacyjne.

**Źródła:**
- https://pl.wikipedia.org/wiki/Praca_organiczna

**Analiza niepoprawnych odpowiedzi:**
- Karol Marcinkowski — dołączył do ruchu po upadku powstania listopadowego (1831)
- Hipolit Cegielski — był działaczem z późniejszego okresu XIX wieku
- Edward Raczyński — znany głównie z fundacji biblioteki i wodociągów w Poznaniu

---

## Pytanie 2

**Question ID:** Q-RO-PO-002

**Pytanie:** Która organizacja została założona w 1857 roku w Poznaniu i stała się ważnym centrum działalności organiczników?

**A)** Spółka Akcyjna Bazar z 1838 roku
**B)** Poznańskie Towarzystwo Przyjaciół Nauk
**C)** Centralne Towarzystwo Rolnicze
**D)** Liga Polska w Poznaniu

**Poprawna odpowiedź:** B

**Wyjaśnienie:** Poznańskie Towarzystwo Przyjaciół Nauk zostało założone w 1857 roku i stało się ważną instytucją skupiającą polskich inteligentów i działaczy społecznych, prowadzącą działalność naukową i oświatową.

**Źródła:**
- https://pl.wikipedia.org/wiki/Praca_organiczna

**Analiza niepoprawnych odpowiedzi:**
- Spółka Akcyjna Bazar z 1838 roku — powstała wcześniej, w 1838 roku
- Centralne Towarzystwo Rolnicze — organizacja rolnicza z lat 60. XIX wieku
- Liga Polska w Poznaniu — partia polityczna z późniejszego okresu

---

## Pytanie 3

**Question ID:** Q-RO-PO-003

**Pytanie:** W którym zaborze praca organiczna rozpoczęła się najwcześniej i rozwinęła najbardziej?

**A)** W zaborze rosyjskim
**B)** W zaborze pruskim
**C)** W zaborze austriackim
**D)** W Księstwie Warszawskim

**Poprawna odpowiedź:** B

**Wyjaśnienie:** Praca organiczna rozpoczęła się najwcześniej w zaborze pruskim, dzięki reformie agrarnej (1811–1823), która uwłaszczyła chłopów, oraz wprowadzeniu powszechnego szkolnictwa. To sprzyjało rozwojowi nowoczesnych idei.

**Źródła:**
- https://pl.wikipedia.org/wiki/Praca_organiczna

**Analiza niepoprawnych odpowiedzi:**
- Zabór rosyjski — praca organiczna rozwinęła się tu dopiero po upadku powstania styczniowego
- Zabór austriacki — miał specyficzne warunki autonomii galicyjskiej
- Księstwo Warszawskie — nie istniało już w okresie pracy organicznej

---

## Pytanie 4

**Question ID:** Q-RO-PO-004

**Pytanie:** Co pozytywiści rozumieli przez hasło "bogaćcie się", które było ważnym elementem pracy organicznej?

**A)** Pomnażanie majątku osobistego i tworzenie miejsc pracy
**B)** Należność do bogatych organizacji politycznych
**C)** Gromadzenie bogactwa kulturalnego w muzeach
**D)** Gromadzenie oszczędności w bankach spółdzielczych

**Poprawna odpowiedź:** A

**Wyjaśnienie:** Pozytywiści uważali, że należy pomnażać osobisty majątek i tworzyć miejsca pracy dla uboższych. To było realizacją hasła "rozumnego egoizmu" — bogacenie się służyło całemu społeczeństwu.

**Źródła:**
- https://pl.wikipedia.org/wiki/Praca_organiczna

**Analiza niepoprawnych odpowiedzi:**
- Organizacje polityczne — praca organiczna unikała polityki na rzecz działalności gospodarczej
- Muzea — nie były głównym celem ruchu
- Banki spółdzielcze — były ważnym elementem, ale nie celem samym w sobie

---

## Pytanie 5

**Question ID:** Q-RO-PO-005

**Pytanie:** Który działacz, znany z założenia fabryki maszyn rolniczych w Poznaniu, był jednym z najważniejszych organiczników Wielkopolski?

**A)** Karol Marcinkowski
**B)** Maksymilian Jackowski
**C)** Hipolit Cegielski
**D)** August Cieszkowski

**Poprawna odpowiedź:** C

**Wyjaśnienie:** Hipolit Cegielski był wybitnym działaczem gospodarczym, który zbudował imperium produkujące maszyny rolnicze. Jego fabryka w Poznaniu stała się symbolem polskiego sukcesu gospodarczego pod zaborami.

**Źródła:**
- https://pl.wikipedia.org/wiki/Praca_organiczna

**Analiza niepoprawnych odpowiedzi:**
- Karol Marcinkowski — prowadził głównie działalność medyczną i edukacyjną
- Maksymilian Jackowski — organizer ruchu rolniczego, nie fabrykant
- August Cieszkowski — filozof, teoretyk pozytywizmu

---

**Podsumowanie pliku:**
- Liczba pytań: 5
- Zakres tematyczny: postawy społeczeństwa polskiego wobec zaborców (podstawa programowa szkoły podstawowej)
- Ograniczenie: Pytania dotyczą tylko podstawowych faktów związanych z pracą organiczną, zgodnie z ograniczonym zakresem w podstawie programowej dla szkoły podstawowej
